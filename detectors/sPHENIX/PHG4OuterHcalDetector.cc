...
PHG4OuterHcalDetector::ConstructSteelPlate(G4LogicalVolume *hcalenvelope)
{
  ...
  steel_plate_uncut = new G4ExtrudedSolid("SteelPlateUnCut",
				    ...
				    zero, 1.0,
				    zero, 1.0);
  m_VolumeSteel += steel_plate_uncut->GetCubicVolume() * (((m_NumScintiPlates/32) - 2)*m_NumSectors);
...
void PHG4OuterHcalDetector::ConstructMe(G4LogicalVolume *logicWorld)
{
  ...
  G4VSolid *hcal_envelope_cylinder = new G4Tubs("OuterHcal_envelope_solid", m_EnvelopeInnerRadius, m_EnvelopeOuterRadius, m_EnvelopeZ / 2., m_StartingSector*M_PI/16, m_StartingSector*M_PI/16 + m_NumSectors*M_PI/16);
