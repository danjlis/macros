...
double get_effs_em(double &angle_t, int &pass, std::vector<float> &pphi, std::vector<float> &peta, int &n, float E[1000], float phi[1000], float eta[1000], TH2F *hetaphi, float &e_thresh){
  int n_phi = 0;
  bool debug = false;
  for (int i = 0; i < n; i++){
    float iphi = phi[i];
  ...
  for (int i = 1; i < 24; i++){
    for (int j = 1; j < 32; j++){
      ...
      if ((pass_p == 0)&&((s1 > e_thresh)||(s2 > e_thresh)||(s3 > e_thresh)||(s4 > e_thresh))) {
        pass_p++;
        phi_p = (hetaphi->GetYaxis()->GetBinCenter(j) + hetaphi->GetYaxis()->GetBinCenter(j+1))/2;
        //std::cout<<"phi_p one: " <<phi_p<<std::endl;
        //if (debug) std::cout<<"phi_p one index: " <<j<<" , "<<i<<std::endl;
      ...
      if (((j > (j_p + 2))||(i > (i_p + 2))||(j < (j_p - 2))||(i < (i_p - 2)))&&(pass_p > 0)&&((s1 > e_thresh)||(s2 > e_thresh)||(s3 > e_thresh)||(s4 > e_thresh))) {
        pass_p++;
        //if (debug) std::cout<<"phi_p in it: " <<phi_p<<std::endl;
        //if (debug) std::cout<<"phi_p index: " <<j<<" , "<<i<<std::endl;

      }
      int jj = 65 - j;
      b1 = hetaphi->GetBinContent(i,jj);
      b2 = hetaphi->GetBinContent(i + 1,jj);
      b3 = hetaphi->GetBinContent(i,jj - 1);
      b4 = hetaphi->GetBinContent(i + 1,jj - 1);

      x1 = b1 + b2;
      x2 = b1 + b3;
      x3 = b2 + b4;
      x4 = b3 + b4;
      if ( b1 >b2 && b1 > b3 && b1 > b4) phi_n = hetaphi->GetYaxis()->GetBinCenter(jj);
      if ( b2 >b1 && b2 > b3 && b2 > b4) phi_n = hetaphi->GetYaxis()->GetBinCenter(jj);
      if ( b3 >b1 && b3 > b2 && b3 > b4) phi_n = hetaphi->GetYaxis()->GetBinCenter(jj-1);
      if ( b4 >b1 && b4 > b3 && b4 > b3) phi_n = hetaphi->GetYaxis()->GetBinCenter(jj-1);

      if ((pass_n == 0)&&((x1 > e_thresh)||(x2 > e_thresh)||(x3 > e_thresh)||(x4 > e_thresh))) {
        pass_n++;
	
        //phi_n = (hetaphi->GetYaxis()->GetBinCenter(jj) + hetaphi->GetYaxis()->GetBinCenter(jj - 1))/2;
        max_phi = phi_n;
        if (x1 > x2 && x1 > x3 && x1 > x4 ) e_max = x1;
        else if (x2 > x1 && x2 > x3 && x2 > x4 ) e_max = x2;
        else if (x3 > x2 && x3 > x2 && x3 > x4 ) e_max = x3;
        else if (x4 > x2 && x4 > x3 && x4 > x3 ) e_max = x4;

        if (debug) std::cout<<"phi_n one: " <<phi_n<<std::endl;
        if (debug) std::cout<<"phi_n one index: " <<jj<<" , "<<i<<std::endl;
        j_n = jj;
        i_n = i;
      }
      else if (((jj > (j_n + 2))||(i > (i_n + 2))||(jj < (j_n - 2))||(i < (i_n - 2)))&&(pass_n > 0)&&((x1 > e_thresh)||(x2 > e_thresh)||(x3 > e_thresh)||(x4 > e_thresh))) {
        pass_n++;
        if (debug) std::cout<<"phi_n in it: " <<phi_n<<std::endl;
        if (debug) std::cout<<"phi_n index: " <<jj<<" , "<<i<<std::endl;
        if (x1 > e_max || x2 > e_max ||x3 > e_max||x4 > e_max) max_phi = phi_n;
      }
    }
  }
  ...
  double phi_t = 2*TMath::Pi() - (fabs(phi_p) + fabs(phi_n));
  if (debug) std::cout<<"phi_p: " <<phi_p<<std::endl;
  if (debug) std::cout<<"phi_n: " <<phi_n<<std::endl;
  if (debug) std::cout<<"phi_t: " <<phi_t<<std::endl;
  angle_t = phi_t;
  double DCA = fabs(224.005*cos(phi_t/2));
  double b = 95.;
  if (DCA < b) {
      pass = 1;
      if (debug) std::cout<<"GOAL!"<<std::endl;
...
void trigger_analysis(){
  ...
  // write file
  TString namef;
  if (use_sim ==1) namef = "hcal_trigger_sim_hist_4GeV_100k1.root";
  ...
  int tower_n;
  int event_num;
  t_out->Branch("event_num", &event_num, "event_num/I");
  t_out->Branch("tower_sim_n", &tower_n, "tower_sim_n/I");
  t_out->Branch("E_thresh", &e_thresh,"e_thresh/I");
  t_out->Branch("truth_vx", &truth_vx,"truth_vx/F");
  t_out->Branch("truth_vy", &truth_vy,"truth_vy/F");
  t_out->Branch("truth_vz", &truth_vz,"truth_vz/F");
  t_out->Branch("part_px", &part_px,"part_px/F");
  t_out->Branch("part_py", &part_py,"part_py/F");
  t_out->Branch("part_pz", &part_pz,"part_px/F");
  t_out->Branch("tower_sim_E", tower_E,"tower_sim_E[tower_sim_n]/F");
  t_out->Branch("tower_sim_eta", tower_eta,"tower_sim_eta[tower_sim_n]/F");
  t_out->Branch("tower_sim_phi", tower_phi,"tower_sim_phi[tower_sim_n]/F");
  ...
  TProfile *h_eff_phi_emcal_all[3];
  ...
  for (int i = 0; i < 3; i++){
    ...
    h_eff_phi_emcal[i] = new TProfile(Form("h_eff_phi_emcal%d", i),"h_eff_phi_emcal", 31, TMath::Pi()/64, 63*TMath::Pi()/64);
    h_phi_p[i] = new TH1D(Form("h_phi_p%d", i),"h_phi_p", 31, TMath::Pi()/64, 63*TMath::Pi()/64);
    h_phi_t_p[i] = new TH2D(Form("h_phi_t_p%d", i),"h_phi_t_p", 31, TMath::Pi()/64, 63*TMath::Pi()/64, 62, TMath::Pi()/64, 2*63*TMath::Pi()/64);
    h_eff_phi_emcal_all[i] = new TProfile(Form("h_eff_phi_emcal_all%d", i),"h_eff_phi_emcal_all", 31, TMath::Pi()/64, 63*TMath::Pi()/64);
  ...
  int end = nen;
  int only_em = 1;
  int emcal_pass_count[3] = {0};
  int emcal_count[3] = {0};
  int all_count[3] = {0};

  float e_t[3] = {2., 5., 10.};
  float e_tt[3] = {2., 10., 20.};
  float b_range[3] = {95, 75, 55};
  if (use_sim == 0) {
    e_t[0] = 100;
    e_t[1] = 300;
    e_t[2] = 700;
  ...
  for (int j = ev; j < end; j++){
    if ((j%10000) == 0) std::cout<<"Event "<<j<<": "<<std::endl;
    event_num = j;
    t->GetEntry(j);
    e_thresh = 0;
    t_out->Fill();
    hetaphi = new TH2F(Form("hetaphi_%d",j),"Muon Path; #eta; #phi",24, -1.1, 1.1, 64, -1*TMath::Pi(), TMath::Pi());
    for ( int i = 0; i < tower_n; i++){
      hetaphi->Fill(tower_eta[i], tower_phi[i], tower_E[i] * 1000);
    }
    for (int i = 0; i < 3; i++){
      if (j%10000 == 0 && emcal_count[i] != 0) std::cout<<"Energy Thresh: "<<i<<":"<<emcal_pass_count[i]<<" / " <<emcal_count[i]<< " = "<<(float)emcal_pass_count[i]/(float)emcal_count[i]<<std::endl;
      ...
      double phi_t;
      double phi_em = get_effs_em(phi_t, pass_em, pphi_em, peta_em, tower_n, tower_E, tower_phi, tower_eta,hetaphi, e_t[i]);
      if (only_em == 0){
      ...
      if (pass_em == 1){
        ...
        h_eff_phi_emcal_all[i]->Fill(phi_em, 1, 1);
        h_phi_p[i]->Fill(phi_em);
        h_phi_t_p[i]->Fill(phi_em, phi_t);
        emcal_pass_count[i]++;
        emcal_count[i]++;
        all_count[i]++;
	e_thresh = i+1;
	...
	//std::cout<<phi_em<<std::endl;
        //hetaphi->Write();

      }
      else if (pass_em == 0){
        h_eff_phi_emcal[i]->Fill(phi_em, 0, 1);
        h_eff_phi_emcal_all[i]->Fill(phi_em, 0, 1);
        h_phi_p[i]->Fill(phi_em);
        h_phi_t_p[i]->Fill(phi_em, phi_t);
        emcal_count[i]++;
        all_count[i]++;
  ...
  for (int i = 0; i < 3; i++){
    std::cout<<"Energy Thresh: "<<i<<":"<<emcal_pass_count[i]<<" / " <<emcal_count[i]<< " = "<<(float)emcal_pass_count[i]/(float)emcal_count[i]<<std::endl;
  }
  h_eff_et->Write();
  h_et_dphi->Write();
  h_eff_et_a->Write();
