...
PHG4OuterHcalDetector::PHG4OuterHcalDetector(PHG4Subsystem *subsys, PHCompositeNode *Node, PHParameters *parames, const std::string &dnam)
  ...
  , m_EnvelopeInnerRadius(0)
  , m_EnvelopeOuterRadius(4000)
  , m_EnvelopeZ(m_SizeZ + 100)
  , m_VolumeEnvelope(NAN)
  , m_VolumeSteel(0)
  , m_VolumeScintillator(NAN)
  , m_NumScintiPlates(m_Params->get_int_param(PHG4HcalDefs::scipertwr) * 2 * m_Params->get_int_param("n_sectors"))
  , m_NumScintiTiles(m_Params->get_int_param("n_scinti_tiles"))
  , m_NumSectors(m_Params->get_int_param("n_sectors"))
  , m_StartingSector(m_Params->get_int_param("n_startingsector"))
...
PHG4OuterHcalDetector::ConstructScintillatorBox(G4LogicalVolume *hcalenvelope)
{
  ...
  m_VolumeScintillator = scintibox->GetCubicVolume() * m_NumScintiPlates*m_NumSectors/32;
  return scintibox;
}
/*
G4VSolid *
PHG4OuterHcalDetector::ConstructSteelPlateHalf(G4LogicalVolume *hcalenvelope)
{
  ...
  if (_debug) std::cout<<"Extruding Solid..."<<std::endl;
  G4VSolid *steel_plate_uncut = new G4ExtrudedSolid("SteelPlateUnCutHalf",
                                                    vertexes,
                                                    m_SizeZ / 4.0,
                                                    zero, 1.0,
                                                    zero, 1.0);
  if (_debug) std::cout<<"Made Extruded solid"<<std::endl;
  //m_VolumeSteel += steel_plate_uncut->GetCubicVolume() * m_NumScintiPlates/10;
...
PHG4OuterHcalDetector::ConstructSteelPlateHalf(G4LogicalVolume *hcalenvelope)
{
  ...
  double xcoord = m_ScintiGap / 2. * cos(angle_mid_scinti / rad) + mid_radius;
  double ycoord = m_ScintiGap / 2. * sin(angle_mid_scinti / rad) + 0;
  std::cout<<"xlow/ylow: "<<xcoord<<" / "<<ycoord<<std::endl;
  PHG4OuterHcalDetector::Point_2 p_loweredge(xcoord, ycoord);
  Line_2 s2(p_in_1, p_loweredge);               // center vertical
  Line_2 perp = s2.perpendicular(p_loweredge);  // that is the lower edge of the steel plate
  PHG4OuterHcalDetector::Point_2 sc1(m_InnerRadius, 0), sc2(0, m_InnerRadius), sc3(-m_InnerRadius, 0);
  ...
  double phi_midpoint = m_NumSectors*(M_PI/16)/ m_NumScintiPlates;
  std::cout<<"phi of scint up: "<<phi_midpoint<<std::endl;
  double xmidpoint = cos(phi_midpoint) * mid_radius;
  double ymidpoint = sin(phi_midpoint) * mid_radius;
  std::cout<<"xhighscint/yhighscint: "<<xmidpoint<<" / "<<ymidpoint<<std::endl;

  // angle of perp line at center of scintillator
  angle_mid_scinti = (M_PI / 2. - phi_midpoint) - (M_PI / 2. + m_TiltAngle / rad);
  double xcoordup = xmidpoint - m_ScintiGap / 2. * sin(angle_mid_scinti / rad);
  double ycoordup = ymidpoint - m_ScintiGap / 2. * cos(angle_mid_scinti / rad);
  std::cout<<"xup/yup: "<<xcoordup<<" / "<<ycoordup<<std::endl;
  PHG4OuterHcalDetector::Point_2 upperleft;
  PHG4OuterHcalDetector::Point_2 upperright;
  double xcoordmid = xcoord + (xcoordup-xcoord)/2;
  double ycoordmid = ycoord + (ycoordup - ycoord)/2;
  std::cout<<"xmid/ymid: "<<xcoordmid<<" / "<<ycoordmid<<std::endl;

  PHG4OuterHcalDetector::Point_2 mid_steel(xcoordmid, ycoordmid);
  PHG4OuterHcalDetector::Point_2 mid_upperscint(xmidpoint, ymidpoint);
  PHG4OuterHcalDetector::Point_2 p_upperedge(xcoordup, ycoordup);
  {
    Line_2 sup(p_loweredge, p_upperedge);       // center vertical
    Line_2 perp = sup.perpendicular(mid_steel);  // that is the upper edge of the steel plate
    PHG4OuterHcalDetector::Point_2 sc1(m_InnerRadius, 0), sc2(0, m_InnerRadius), sc3(-m_InnerRadius, 0);
    Circle_2 inner_circle(sc1, sc2, sc3);
    vector<CGAL::Object> res;
    CGAL::intersection(inner_circle, perp, std::back_inserter(res));
  ...
  // to be a tangent at the center
  ShiftSecantToTangent(lowerleft, upperleft, upperright, lowerright);
  std::cout<<"lowerleft: "<<CGAL::to_double(lowerleft.x())<<" / "<<CGAL::to_double(lowerleft.y())<<std::endl;
  std::cout<<"lowerright: "<<CGAL::to_double(lowerright.x())<<" / "<<CGAL::to_double(lowerright.y())<<std::endl;
  std::cout<<"upperleft: "<<CGAL::to_double(upperleft.x())<<" / "<<CGAL::to_double(upperleft.y())<<std::endl;
  std::cout<<"upperright: "<<CGAL::to_double(upperright.x())<<" / "<<CGAL::to_double(upperright.y())<<std::endl;
  ...
  vertexes.push_back(v4);
  G4TwoVector zero(0, 0);
  G4VSolid *steel_plate_uncut;
  steel_plate_uncut = new G4ExtrudedSolid("SteelPlateUnCut",
				    vertexes,
				    m_SizeZ / 2.0,
				    zero, 1.0,
				    zero, 1.0);
  m_VolumeSteel += steel_plate_uncut->GetCubicVolume() * 2*m_NumSectors;
  ...
  G4RotationMatrix *rotm = new G4RotationMatrix();
  rotm->rotateX(-90 * deg);
  G4VSolid *steel_firstcut_solid = new G4SubtractionSolid("SteelPlateFirstCutHalf", steel_plate_uncut, m_SteelCutoutForMagnetG4Solid, rotm, G4ThreeVector(0, 0, 0));
  delete rotm;
  //   DisplayVolume(steel_plate_uncut, hcalenvelope);
  //    DisplayVolume(m_SteelCutoutForMagnetG4Solid, hcalenvelope);
  //    DisplayVolume(m_SteelCutoutForMagnetG4Solid, hcalenvelope,rotm);
  ...
  rotm = new G4RotationMatrix();
  rotm->rotateX(90 * deg);
  G4VSolid *steel_cut_solid = new G4SubtractionSolid("SteelPlateCutHalf", steel_firstcut_solid, m_SteelCutoutForMagnetG4Solid, rotm, G4ThreeVector(0, 0, 0));
  // DisplayVolume(steel_cut_solid, hcalenvelope);
  delete rotm;
  return steel_cut_solid;
}
...
PHG4OuterHcalDetector::ConstructSteelPlate(G4LogicalVolume *hcalenvelope)
{
  ...
  steel_plate_uncut = new G4ExtrudedSolid("SteelPlateUnCut",
				    ...
				    zero, 1.0,
				    zero, 1.0);
  m_VolumeSteel += steel_plate_uncut->GetCubicVolume() * (m_NumScintiPlates/32)*m_NumSectors;
...
void PHG4OuterHcalDetector::ConstructMe(G4LogicalVolume *logicWorld)
{
  ...
  G4VSolid *hcal_envelope_cylinder = new G4Tubs("OuterHcal_envelope_solid", m_EnvelopeInnerRadius, m_EnvelopeOuterRadius, m_EnvelopeZ / 2., m_StartingSector*M_PI/16, m_StartingSector*M_PI/16 + 32*M_PI/16);
...
int PHG4OuterHcalDetector::ConstructOuterHcal(G4LogicalVolume *hcalenvelope)
{
  ...
  if(_debug) std::cout<<"Constructing half plate"<<std::endl;
  //G4VSolid *steel_plate_half = ConstructSteelPlateHalf(hcalenvelope);
  ...
  //G4LogicalVolume *steel_logical_half = new G4LogicalVolume(steel_plate_half, G4Material::GetMaterial(m_Params->get_string_param("material")), "HcalOuterSteelPlateHalf", 0, 0, 0);
  //m_DisplayAction->AddSteelVolume(steel_logical_half);
  m_DisplayAction->AddSteelVolume(steel_logical);
  double phi = m_StartingSector*M_PI/16;
  double deltaphi = m_NumSectors*(M_PI/16) / m_NumScintiPlates;
  //double half_deltaphi = deltaphi / 2;
  ostringstream name;
  ...
  int steel_plate_index = 1;
  int steel_plate_num = 0;
  // else (for m_TiltAngle = 0) phi stays zero
  for (int i = 0; i < m_NumScintiPlates; i++)
  {
    std::cout<<"Steel Plate Index: "<<steel_plate_index<<std::endl;
    std::cout<<"Phi: "<<phi<<std::endl;
/*
    if (steel_plate_index == 0) {
      G4RotationMatrix *Rot = new G4RotationMatrix();
      Rot->rotateZ(-phi * rad);
      name.str("");
      name << "OuterHcalSteel_" << steel_plate_num;
      m_SteelAbsorberVec.insert(new G4PVPlacement(Rot, G4ThreeVector(0, 0, 0), steel_logical_half, name.str().c_str(), hcalenvelope, 0, steel_plate_num, OverlapCheck()));
      delete Rot;
      phi += half_deltaphi;
      steel_plate_index++;
      steel_plate_num++;
...
*/
    ...
    name.str("");
    /*
    if (steel_plate_index == 10) {
      name << "OuterHcalSteel_" << steel_plate_num;
      m_SteelAbsorberVec.insert(new G4PVPlacement(Rot, G4ThreeVector(0, 0, 0), steel_logical_half, name.str().c_str(), hcalenvelope, 0, steel_plate_num));
      steel_plate_index = 0;
      steel_plate_num++;
      phi += half_deltaphi;
    }
  ...
  //steel_logical_half->SetFieldManager(m_FieldSetup->get_Field_Manager_Iron(), true);
...
int PHG4OuterHcalDetector::ConsistencyCheck() const
{
  ...
  {
    cout << PHWHERE << "Scintillator inner radius " << m_ScintiInnerRadius / cm
  ...
  {
    cout << PHWHERE << "Magnet scintillator cutout radius " << m_Params->get_double_param("magnet_cutout_scinti_radius")
  ...
  {
    cout << PHWHERE << "Magnet steel cutout radius " << m_Params->get_double_param("magnet_cutout_radius")
         << " cm smaller than inner radius " << m_InnerRadius / cm
         << " cm" << endl;
    gSystem->Exit(1);
  }
